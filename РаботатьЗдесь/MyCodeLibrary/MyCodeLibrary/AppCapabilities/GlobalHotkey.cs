...
namespace MyCodeLibrary
{
    ...
    //Глобальный хоткей это комбинация клавиш вроде CTRL+K или WIN+F1.
    //Сначала приложение должно ее для себя зарегистрировать.
    //Можно зарегистрировать несколько хоткеев для одного окна. 
    //Но зарегистрировать можно только для себя, в том же потоке, который обслуживает само окно.
    //Для этого нужен описатель существующего окна, и некоторое произвольное число-идентификатор хоткея.
    //По этому идентификатору хоткей можно определить и обработать в процедуре окна.

    //Когда окно закрывается, глобальный хоткей разрегистрируется (вроде бы) операционной системой.
    //Но правильно будет разрегистрировать его явно при обработке закрытия окна.

    //Глобальный хоткей позволяет вызвать окно из любого места и момента в пределах Рабочего стола и сеанса пользователя.
    //Даже если в текущем приложении определен такой же, но локальный, хоткей, например, CTRL+A, 
    //при нажатии клавиш сработает глобальный хоткей и будет вызвана оконная процедура, 
    //зарегистрировавшая этот глобальный хоткей. А не локальный обработчик текущего приложения.

    //Обычно эта процедура должна показать свое окно пользователю. 
    //Иногда его надо развернуть из таскбара или из трея.
    //Но эти операции приложение не выполняет автоматически - это 
    //должен делать код обработчика сообщения хоткея, который нужно написать.
    //Приложение может и не показывать окно, а просто сделать некоторую работу.
