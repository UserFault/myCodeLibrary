...
namespace MyCodeLibrary
{
    ...
    public class CRC32st
    {
        ...
        {
            return (uint)(CRC32st.CrcTable[(oldCrc ^ value) & 0xFF] ^ (oldCrc >> 8));
        }
        ...
        {
            m_crc ^= CrcSeed;
            m_crc = CrcTable[(m_crc ^ value) & 0xFF] ^ (m_crc >> 8);
            m_crc ^= CrcSeed;
        }
        ...
        public void Update(byte[] buffer, int offset, int count)
        {
            ...
            while (--count >= 0)
            {
                m_crc = CrcTable[(m_crc ^ buffer[offset++]) & 0xFF] ^ (m_crc >> 8);
            }
        ...
        public UInt32 GetCrc32AndCopy(System.IO.Stream input, System.IO.Stream output)
        {
            if (input == null)
                ...
                m_totalBytesRead = 0;
                int count = input.Read(buffer, 0, 16384);
                if (output != null) output.Write(buffer, 0, count);
                m_totalBytesRead += count;
                while (count > 0)
                {
                    Update(buffer, 0, count);
                    count = input.Read(buffer, 0, 16384);
                    if (output != null) output.Write(buffer, 0, count);
                    m_totalBytesRead += count;
                }
