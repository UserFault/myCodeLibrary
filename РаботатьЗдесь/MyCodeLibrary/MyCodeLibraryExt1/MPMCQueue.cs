...
//Очередь много писателей много читателей
//Не тестировалась мною!
...
namespace MyCodeLibraryExt1
{
    ...
    public class MPMCQueue
    {
        ...
        {
            if (bufferSize < 2) throw new ArgumentException("Argument should be greater than or equal to 2", "buffersize");
            if ((bufferSize & (bufferSize - 1)) != 0) throw new ArgumentException("Argument should be a power of 2", "buffersize");
        ...
        public bool TryEnqueue(object item)
        {
            do
            {
                ...
                if (cell.Sequence == pos && Interlocked.CompareExchange(ref _enqueuePos, pos + 1, pos) == pos)
                {
                    ...
                    Thread.VolatileWrite(ref buffer[index].Sequence, pos + 1);//кажется, так правильно?
                    return true;
                }
        ...
        public bool TryDequeue(out object result)
        {
            do
            {
                ...
                if (cell.Sequence == pos + 1 && Interlocked.CompareExchange(ref _dequeuePos, pos + 1, pos) == pos)
                {
                    ...
                    Thread.VolatileWrite(ref buffer[index].Sequence, pos + bufferMask + 1);//кажется, так правильно?
                    return true;
                }
...
//### Overview
//This is an attempt to port [the famous Bounded MPMC queue algorithm by Dmitry Vyukov][1024-mpmc] to .NET. 
//All credit goes to Dmitry Vyukov. I let myself quote the description:

//>According to the classification it's MPMC, array-based, fails on overflow, does not require GC, 
//w/o priorities, causal FIFO, blocking producers and consumers queue. 
//The algorithm is pretty simple and fast. It's not lockfree in the official meaning, 
...
//Producers and consumers are separated from each other (as in the two-lock queue), i.e. do not touch 
//the same data while queue is not empty.

//### Implementation

//The queue class layout is shown below. The `_buffer` field stores enqueued elements and their sequences. 
//It has size that is a power of two. The `_bufferMask` field is used to avoid the expensive modulo 
//operation and use `AND` instead. There's padding applied to avoid [false sharing][false-sharing] 
//of `_enqueuePos` and `_dequeuePos` counters. And [Volatile.Read/Write to suppress memory instructions 
//reordering when read/write cell.Sequence][memory-barriers-in-dot-net].
